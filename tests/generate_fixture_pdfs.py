...
def _draw_placement_tests(
    c: canvas.Canvas, tests: list[tuple[str, str, str]], y: float
) -> float:
...
def _draw_transfer_block(
    c: canvas.Canvas, credits: list[tuple[str, str, str, str, str, str]], y: float
) -> float:
    ...
    c.setFont(_FONT, 5)
    c.drawString(_X, y, "EXTENSION/INDEPENDENT STDY/ADVANCE PLACEMENT CREDIT:")
    y -= _LINE
    c.setFont(_FONT, _FONT_SIZE)
    current_source = None
...
def _draw_summary(
    c: canvas.Canvas, gpa: float, total_credits: float, uw_credits: float, y: float
) -> float:
    """Draw the CUMULATIVE CREDIT SUMMARY block."""
    c.drawString(_X, y, "CUMULATIVE CREDIT SUMMARY:")
    y -= _LINE
    c.drawString(_X, y, f"UW CREDITS EARNED {uw_credits:.1f}")
    y -= _LINE
    c.drawString(_X, y, f"UW GRADE POINT AVG. {gpa:.2f}")
    y -= _LINE
    c.drawString(_X, y, f"CREDITS EARNED {total_credits:.1f}")
    y -= _LINE + 6
    return y


def _new_page_if_needed(
    c: canvas.Canvas, y: float, margin: float = 1.0 * inch
) -> float:
...
def generate_csse_junior(out_dir: Path) -> Path:
    ...
    y = _draw_header(
        ...
        "2429082",
        "Bothell",
        "COMP SCI & SOFTWARE ENGR",
        "JUNIOR",
        "SPRING QUARTER, 2026",
        y,
    )

    y = _draw_placement_tests(
        c,
        [
            ("FYCDSP", "01", "07/31/24"),
            ("MTHDSP", "500", "05/06/24"),
